...
if ((_HQ getVariable ["RydHQ_Debug",false]) or (isPlayer (leader _unitG))) then 
	...
	_i = [[_posX,_posY],_unitG,"markAttack","ColorRed","ICON","mil_dot","Snp " + _signum," - ATTACK"] call RYD_Mark
	};
...
if (isPlayer (leader _unitG)) then
	...
	if not (isMultiplayer) then
		...
		[_task,(leader _unitG),["Stay hidden, where you are and snipe any spotted enemy units.", "Snipe", ""],(getPosATL _Trg),"ASSIGNED",0,false,true] call BIS_fnc_SetTask;
		}
	};
...
if not (_alive) exitwith {[_Trg,"SnpAttacked"] call RYD_VarReductor;if ((_HQ getVariable ["RydHQ_Debug",false]) or (isPlayer (leader _unitG))) then {deleteMarker ("markAttack" + str (_unitG))}};
...
if (isPlayer (leader _unitG)) then
	...
	if not (isMultiplayer) then
		...
		{
		[_task,(leader _unitG),["Withdraw.", "Move", ""],_Spos,"ASSIGNED",0,false,true] call BIS_fnc_SetTask;
